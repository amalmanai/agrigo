...
public class ListeTacheController {
    ...
    @FXML
    private TableView<Tache> ListTache;
    @FXML
    private Label labelMeteo;
    @FXML
    private Label labelCitation;
    @FXML
    private TextField fieldRechercheTache;
    @FXML
    private ComboBox<String> comboFiltreStatut;
    @FXML
    private Label labelAucuneTache;

    private final ServiceTache serviceTache = new ServiceTache();
    private ObservableList<Tache> taches;
    private ObservableList<Tache> tachesFull;
    /**
     * Si true, affiche uniquement les tâches de l'utilisateur connecté (Mes
     * tâches).
     */
    ...
    public void initialize() {
        ...
        ListTache.getColumns().addAll(colTitre, colDesc, colType, colDate, colDebut, colFin, colStatus, colRemarque,
                colAction);
        ...
        // Charger les données (selon le rôle : toutes les tâches ou uniquement les
        // siennes)
        loadTaches();
        ...
        // API 2 (Tâche) : Citation aléatoire (avec texte de secours si l'API ne répond
        // pas)
        TacheApiService.getRandomQuote().thenAccept(q -> Platform.runLater(() -> {
            if (labelCitation == null)
                return;
            if (q != null && q.content != null && !q.content.isEmpty()) {
                ...
                labelCitation.setText(
                        "💡 Astuce AGRIGO : notez vos tâches dès qu’elles sont décidées pour garder une trace claire des travaux au champ.");
            }
        }));
    }
    ...
    private void loadTaches() {
        ...
        if (allTaches == null) {
            allTaches = new java.util.HashSet<>();
        }

        taches = FXCollections.observableArrayList(allTaches);
        ListTache.setItems(taches);
        if (tachesFull != null)
            tachesFull.setAll(allTaches);
        mettreAJourPlaceholderTache();
    }
    ...
    private void appliquerFiltreTache() {
        ...
        String statut = comboFiltreStatut != null && comboFiltreStatut.getValue() != null
                ? comboFiltreStatut.getValue().trim()
                : "";
        taches.setAll(tachesFull.stream()
                .filter(t -> (q.isEmpty()
                        || (t.getTitre_tache() != null && t.getTitre_tache().toLowerCase().contains(q)))
    ...
    private void mettreAJourPlaceholderTache() {
        if (labelAucuneTache != null)
            labelAucuneTache.setVisible(taches != null && taches.isEmpty());
    }
    ...
    public void rafraichir(ActionEvent event) {
        ...
        if (fieldRechercheTache != null)
            fieldRechercheTache.clear();
        if (comboFiltreStatut != null)
            comboFiltreStatut.setValue(null);
    }
    ...
    public void retourAccueil(ActionEvent event) {
        try {
            String fxmlPath = "/menu.fxml";
            String title = "Menu Principal";

            if (Session.getCurrentUser() != null && "admin".equalsIgnoreCase(Session.getCurrentUser().getRole_user())) {
                fxmlPath = "/menuadmin.fxml";
                title = "Menu Admin";
            }
            ...
            new javafx.scene.control.Alert(javafx.scene.control.Alert.AlertType.ERROR,
                    "Impossible de revenir à l'accueil.").showAndWait();
        }
    }
    ...
    public void exportPdf(ActionEvent event) {
        ...
        try {
            ...
            new Alert(Alert.AlertType.INFORMATION, selected.size() + " tâche(s) exportée(s) : " + f.getAbsolutePath())
                    .showAndWait();
        } catch (Exception e) {
            e.printStackTrace();
