    ...
    void sendCodeAction(ActionEvent event) {
        ...
        if (rbEmail.isSelected()) {
            ...
            try {
                ...
                showAlert("Code de vérification envoyé par email.");
            } catch (jakarta.mail.MessagingException ex) {
                System.err.println("Erreur envoi email: " + ex.getMessage());
                ex.printStackTrace();
                showAlert("⚠ Erreur lors de l'envoi de l'email. Le code est: " + verificationCode);
                // En développement, on garde le code pour permettre la progression
            }
        } else {
            ...
            verificationCode = generateCode();
            // appeler le service SMS réel
            try {
                String text = "Votre code de vérification AGRIGO : " + verificationCode;
                boolean sent = SmsUtil.sendSms(phone, text);
                if (sent) {
                    System.out.println("Code envoyé par SMS à " + phone + " : " + verificationCode);
                    showAlert("Code de vérification envoyé par SMS.");
                } else {
                    System.out.println("Échec envoi SMS (réponse API négative) pour " + phone);
                    showAlert("Échec de l'envoi du SMS. Veuillez réessayer ou choisir l'email.");
                    // invalidating the code since not delivered
                    verificationCode = null;
                    return;
                }
            } catch (IOException ex) {
                ex.printStackTrace();
                showAlert("Erreur lors de l'envoi SMS : " + ex.getMessage());
                verificationCode = null;
                return;
            }
        }
